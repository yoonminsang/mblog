    ...
    function(error, post) {
      ...
      db.query(`SELECT * FROM category`, function(error2, category) {
        ...
        for (var i = 0; i < post.length; i++) {
          if (
            post[i].title.includes(id) === false &&
            post[i].content.includes(id) === false
          ) {
            delete post[i];
          }
        }
        ...
        var offset = 2 * (id2 - 1);
        db.query(
          `SELECT id, title, content, category_id, date_format(created, '%Y-%m-%d %H:%i:%s') as created FROM post LIMIT 2 OFFSET ${offset}`,
          function(error3, post2) {
            ...
            var pagination = ``;
            for (var i = 0; i < post.length / 2; i++) {
              pagination += `<a href="/search/${id}/page/${i + 1}">${i +
                1}</a>`;
            }
...
router.get("/:pageId/page/:pageId2", function(request, response) {
  ...
  var offset = 2 * (id2 - 1);
  db.query(
    `SELECT id, title, content, category_id, date_format(created, '%Y-%m-%d %H:%i:%s0') as created FROM post WHERE title LIKE '%${id}%' OR content LIKE'%${id}%' order by id DESC LIMIT 2 OFFSET ${offset}`,
    function(error, post) {
      if (error) {
        throw error;
      }
        ...
        function(error2, allPost) {
          ...
          db.query(`SELECT category_id FROM post`, function(error4, listPost) {
            ...
            db.query(`SELECT * FROM category`, function(error3, category) {
              ...
              var title = `'${id}'의 검색결과`;
              var title2 = `<span>${id}</h2></span><span style="padding-left : 5px; font-weight : bold">(${post.length})</span>`;
              var inner = template.postList(post);
              var pagination = ``;
              for (var i = 0; i < post.length / 2; i++) {
                if (id2 == i + 1) {
                  pagination += `<a href="/search/${id}/page/${i +
